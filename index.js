...
router.post('/login', function(req, res) {
        res.send(req.he); //redirect(req.reverse('hello'));
    })
...
function getCollectionAsMarkdown(collectionName) {
    ...
    var markdown = db._query(
        ...
        return CONCAT_SEPARATOR("\n", attributesPerDocument)`, { "@collection": collectionName })
...
router.get('/:htmltemplate', function(req, res) {
        ...
        var result = `<html>
                ...
                <body>
                ${mainTemplateJSON.menu}
                <div id="content">${mainTemplateJSON.body}</div>
                ${mainTemplateJSON.footer}
                </body></html>`
        res.send(result);
    })
    .error(404, 'Document not found.')
